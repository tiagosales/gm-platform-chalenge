node {
    withEnv(['APP_NAME=gmile-app','REGION=us-east-1','CLUSTER=gmile-ecs-cluster','SERVICE_NAME=gmile-service','NUM_TASKS=1']){
        ...
        stage('Deploy on ECS'){
          sh("aws ecs --no-verify-ssl update-service --cluster ${CLUSTER} --region ${REGION} --service ${SERVICE_NAME} --desired-count ${NUM_TASKS} --force-new-deployment --task-definition ${APP_NAME}:${TASKDEF_REV}")
        }
    }
}
