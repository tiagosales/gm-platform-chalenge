node {
    ...
    withEnv(['REPO_NAME=gmile-challenge', 'REGION=us-east-1']){
      stage('Push image') {
        ECR_ADDRESS = sh(script: 'aws ecr --no-verify-ssl describe-repositories --repository-names ${REPO_NAME} --region ${REGION} | grep repositoryUri | awk -F: {\'print $2\'} | tr -d \\",\\ ', returnStdout: true)
